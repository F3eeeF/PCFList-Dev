...
export const getElettricCode = async (context: ComponentFramework.Context<IInputs>) : Promise<string> => {
    const entityName = "emd_licenseplatecountryrule";
    const fetchXml = `<fetch version="1.0" no-lock="true" top="1">
            <entity name="emd_licenseplatecountryrule">
            <attribute name="statecode"/>
            <attribute name="emd_licenseplatecountryruleid"/>
            <attribute name="emd_name"/>
            <attribute name="createdon"/>
            <attribute name="emd_elettricsuffixcode"/>
            <order attribute="emd_name" descending="false"/>
            <filter type="and">
            <condition attribute="statecode" operator="eq" value="0"/>
            <condition attribute="emd_countryid" operator="eq" value="{7a4c9ba3-b22d-f111-88b4-7ced8d484466}" uiname="Austria" uitype="emd_country"/>
            </filter>
            </entity>
            </fetch>`
    ...
    return entities.emd_elettricsuffixcode;
}